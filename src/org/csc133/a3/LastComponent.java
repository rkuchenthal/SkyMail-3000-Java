...
public class LastComponent extends Component {
    ...
    public LastComponent(GameWorld gw) {
        this.gw =gw;
        try {
        ...
        //add all images to clockDigit array
        for (int i = 0; i < numDigitsShowing; i++) {
            clockDigits[i] = digitImages[0];
        }

        //initialize digit colors
        ledColor = ColorUtil.WHITE;
    }

    private void setCurrentCheck() {
        lastSky = gw.getLast();

        //insert digits pics into clock []
        clockDigits[0] = digitImages[lastSky % 10];
